...
class KskPosts extends \yii\db\ActiveRecord
{
     ...
     * @return \yii\db\ActiveQuery
     */
    public function getKskCommentsTree($root = 0)
    {
        $data = $this->getKskComments()->asArray()->all();
        
        /*echo '<div style="margin-top: 70px;"><pre>';
        echo print_r($data);
        echo '</pre></div>';*/
        
        //echo var_dump($this->getKskComments()->prepare(Yii::$app->db->queryBuilder)->createCommand()->getRawSql());
    ...
    public function addComment($comment)
    {
        if(Yii::$app->params['commentNeedApproval'])
            $comment->approved = KskComments::STATUS_PENDING;
        else
            $comment->approved = KskComments::STATUS_APPROVED;
